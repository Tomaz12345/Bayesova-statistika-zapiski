...
\begin{exmp}
  ...
  \begin{itemize}
    \item nimamo apriornega mnenja o (dejanskem) $p$:
      tedaj bi (morda) vzeli zvezno porazdelitev z gostoto enakomerne porazdelitve, \\
...
\begin{align*}
  ...
  &\stackrel{\text{pogojno neodvisne}}{\longeq{40pt}} \int_{0}^{1} P(X_i = 1 \mid \Pi = p) P(X_j = 1 \mid \Pi = p) f(p) dp = \\
  &= \int_{0}^{1} p^2 f(p) dp = \\
  &= E(\Pi^2).
\end{align*}
...
\end{defn}
\begin{defn}
  Pogojno varianco slučajnega vektorja $Y$, pogojno na \\
  $X = x$ definiramo kot varianco pogojne porazdelitve $(Y \mid X = x)$, t.j.
  \begin{equation*}
    E((Y - u(X))(Y - u(X))^T \mid X = x) =: Var(Y \mid X = x).
  \end{equation*}
  Ker je $E$ aditivna, velja
  \begin{equation*}
    E\left((Y - u(X))(Y - u(X))^T \mid X = x\right) = E(Y Y^T \mid X = x) - u(X) u(X)^T =: v(X).
  \end{equation*}
...
To pomeni
\begin{equation*}
  \tau_1^2 = \left(\frac{n}{\sigma^2} + \frac{1}{\tau_0^2}\right)^{-1}
\end{equation*}
in
\begin{align*}
  \mu_1^2 &= \tau_1^2 \left(\frac{n}{\sigma^2} + \frac{1}{\tau_0^2}\right) \\
  &= \frac{\left(\frac{n}{\sigma^2} \overline{x} + \frac{1}{\tau_0^2} \mu_0\right) \cdot \frac{\tau_0^2 \sigma^2}{n}}
    {\left(\frac{n}{\sigma^2} + \frac{1}{\tau_1^2}\right) \cdot \frac{\tau_0^2 \sigma^2}{n}} \\
...
Izkaže se, da v statistični praksi (tudi v aplikacija frekventistične statistike) potrebujemo ta koncept.
...
\begin{exmp}
  ...
  \begin{exmp}
    ...
    \begin{align*}
      ...
      &= \binom{n}{k} (1+e^q)^{-n} e^{kq} \quad (q \in \R).
    \end{align*}
  ...
  Odvajajmo
  \begin{align*}
    \frac{\partial}{\partial \lambda_i} \ln\left(\widetilde{f}(x \mid \lambda)\right) &=
      \sum_{j=1}^{n} \left(\frac{\partial}{\partial \lambda_j} \ln(f(x \mid \lambda_j)) \cdot
        \frac{\partial (\psi^{-1})_j}{\partial \lambda_i} (\lambda)\right) \\
...
\begin{exmp} \text{} \\
  ...
  Če imamo normalni model s parametrom $\theta = (\mu, \sigma^2)$, nas torej zanima \\
  $h(\mu, \sigma^2) = (\mu^2, \sigma^2)$.
...
\begin{equation*}
  S^2 = \frac{1}{n-1} \sum_{i=1}^{n} (h(X_i) - \overline{h(X_.)})^2.
\end{equation*}
Po CLI velja
\begin{equation*}
...
\begin{enumerate}[label={A/R \#\arabic{*}:}]
  \item privzemimo $\{x \mid f(x) > 0\} \subset (a,b)$, kjer $-\infty < a < b < \infty$ in $\exists m: f(x) < m$ za vse $x$. \\
    ...
    \begin{align*}
      P(X \in I) &\stackrel{\text{vaja}}{\longeq{12pt}} P(Y \in I \mid v < f(y)) \\
      &= \frac{\int_{\R} P(Y \in I \land v < f(Y) \mid Y=y) \cdot f_Y(y) dy}
        {\int_{\R} P(v < f(Y) \mid Y=y) \cdot f_Y(y) dy} \\
      &\stackrel{\text{pogojna}}{\longeq{25pt}} \frac{\int_{\R} P(y \in I \land v < f(y)) \cdot f_Y(y) dy}
        {\int_{\R} P(v < f(y)) \cdot f_Y(y) dy} \\
      &\stackrel{I \subset (a,b)}{\longeq{25pt}} \frac{\int_{I} P(v < f(y)) \cdot \frac{1}{b-a} dy}
  ...
  \item Privzemimo, da znamo vzorčiti iz gostote $g: \R \to [0,\infty)$ in da je $f(x) < M \cdot g(x)$ za vse $x$ (za neki $M$).
    ...
    \begin{align*}
      P(X \in I) &\stackrel{\text{vaja}}{\longeq{12pt}} P(Y \in I \mid M \cdot g(y) \cdot w < f(y)) \\
      ...
      &\stackrel{\text{pogojna}}{\longeq{25pt}} \frac{\int_{I} P(W < \frac{f(y)}{Mg(y)}) \cdot g(y) dy}
        {\int_{\R} P(W < \frac{f(y)}{Mg(y)}) \cdot g(y) dy} \\
      &= \dots = \int_I f(y) dy,
    \end{align*}
    kjer smo upoštevali $P\left(W < \frac{f(y)}{Mg(y)}\right) = \frac{f(y)}{Mg(y)}$.
...
\end{equation*}
Vzenimo $a := 1_A$. \\ %a,n?
Tedaj $\frac{\text{število členov (do $n$-tega, ki padejo v) A}}{n} \stackrel{n \to \infty}{\longrightarrow}
\int 1_A(x) f(x) dx = a$. %a?
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item privzamemo, da že imamo realizacijo $X^{(n-1)} = x^{(n-1)}$.
    ...
    \begin{equation*}
      ...
      \end{cases}
    \end{equation*}
    Naenkrat. $X^{(n)} = y$ z verjetnostjo $\rho = \min \left\{\frac{f(y)}{f\left(x^{(n-1)}\right)}, 1\right\}$ (*) \\
    (*): če $f\left(x^{(n-1)}\right) = 0$, vedno vzamemo $y$. \\
    Ta korak implementiramo z realizacijo $u \in U(0,1)$, vzamemo $X^{(n)} = y$,
    če $u \leq \rho$, oz. $X^{(n)} = x^{(n-1)}$, če $u > \rho$.
\end{enumerate}
...
\begin{equation*}
  \rho = \rho\left(x^{(n-1)}, y\right) = \min \left\{1, \frac{f(y)}{f\left(x^{(n-1)}\right)} \cdot
    \frac{q\left(x^{(n-1)} \mid y\right)}{q\left(y \mid x^{(n-1)}\right)}\right\}
\end{equation*}
...
\begin{exmp} \text{} \\
  ...
  privzemimo model $f(x \mid \theta_1, \theta_2)$ z apriorno gostoto $f(\theta_1, \theta_2) = f(\theta_1) \cdot f(\theta_2)$,
...
\begin{exmp} \text{} \\
  ...
  (kjer so $X_0, \varepsilon_1 \dots$ neodvisne in $\varepsilon_0 \sim N(0, V)$ za $V \in P(r)$ (V je s.p.d.))
    ...
    \item če je $X_{n-1} = x$, potem realizacijo od $X_n$ dobimo takole:
      \begin{enumerate}[label=(\roman*)]
        ...
        \item vzorčimo $u = U \sim U(0,1)$ in sprejmemo $X_n = y$, če $u \leq \rho(x, y)$
          ...
          \begin{equation*}
            \rho(x,y) = \begin{cases}
              \min \left\{1, \frac{f(y)}{f(x)} \cdot \frac{q(x \mid y)}{q(y \mid x)}\right\}: &f(x) q(y \mid x) \neq 0 \\
              1: &f(x) q(y \mid x) = 0
            \end{cases}
          \end{equation*}
...
\begin{itemize}
  \item $||P^n(_ \mid x) - \pi|| \stackrel{n \to \infty}{\longrightarrow} 0$ za skoraj vse $x$,
  \item $||P^n(_ \mid x) - \pi|| \stackrel{n \to \infty}{\longrightarrow} 0$ enakomerno v $x$,
\end{itemize}
...
Sledi želeni ECLI: \\
$\lim_{n \to \infty} \frac{\widehat{E_f(h)} n(k) - E_f(h)}{\frac{\sqrt{\widehat{\gamma_k^2}(a(k),b(k))}}{\sqrt{n}}}
  \stackrel{D}{=} N(0,1)$ \\
in še EIZ (ergodični interval zaupanja) (stopnje zaupanja $1 - \alpha$) \\
...
\begin{equation*}
  \widehat{n_{eff}} = n \frac{\widehat{\sigma^2}}{\gamma_k^2} = \frac{\widehat{\sigma^2}}{\frac{\gamma_k^2}{n}}.
\end{equation*}
...
\end{equation*}
Tu je $f(\vartheta_1 \mid \vartheta_2, x)$ aposteriorna gostota modela z vzorčnimi gostotami \\
$f(x \mid \vartheta_2)$ in apriorno gostoto $f(\vartheta_2$). \\
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item Za $f(\sigma^2 \mid x)$ potrebujemo $f(x \mid \sigma^2)$. \\
    ...
    \begin{equation*}
      ...
      \end{bmatrix}.
    \end{equation*}
    Vidimo, da je $(X, M \mid V \sim \sigma^2)$ normalna porazdelitev \\
    $\implies \; (X \mid V \sim \sigma^2)$ je kot robna tudi normalna. \\
...
\begin{itemize}
  ...
  \item $f(\phi \mid \vartheta, x) = \frac{f(x \mid \vartheta) f(\phi \mid \vartheta)}{f(x \mid \vartheta)}$:
    aposteriorna gostota modela z vzorčnimi \\
    $f(\vartheta \mid \phi)$ in apriornimi $f(\phi)$
\end{itemize}
...
\end{rem}
\begin{defn}
  Naj bo $\pi$ stacionarna oprazdelitev verige $\{X_n\}$, ki je \\
  $\pi$-ireducibilna.
...
\begin{theorem}
  ...
  \begin{itemize}
    \item Če je veriga še aperiodična, velja $\lim_{n \to \infty} \sup_A \left|P^n(A \mid x) - \pi(A)\right| = 0$ za vsak $x$
      ($\pi$-s.g. $[x]$).
    \item Če je veriga Harrisov povrnljiva, velja \\
      $\lim_{n \to \infty} \sup_A B\left(P^n(A \mid x) - \pi(A)\right) = 0$ za vsak $x$.
  \end{itemize}
...
\begin{align*}
  &Y_1 = \beta_0 + X_{11} \beta_1 + \dots + X_{1p} \beta_p + \varepsilon_1 \\
  &\vdots \\
  &Y_n = \beta_0 + X_{n1} \beta_1 + \dots + X_{np} \beta_p + \varepsilon_n. \\
\end{align*}
Tu do $X_{ij} \; (1 \leq i \leq n, \; 1 \leq j \leq p)$ ZNANE (nastavljene) konstante,
$\varepsilon_1 \dots \varepsilon_n$ pa so slučajne SPREMENLJIVKE. \\
Če označimo
\begin{align*}
  &y = \begin{bmatrix}
    y_1 \\
  ...
  \end{bmatrix}_{n \times 1}
\end{align*}
sledi enačba linearne regresije $y = X \beta + \varepsilon$. \\
\underline{MNK} \\
...
$\varepsilon$ slučajen s porazdelitvijo v nekem modelu. \\
Vedno privzamemo $E(\varepsilon) = 0 \in \R^m$ (če $\neq 0$: sistemska napaka). \\
Za variančno-kovariančno matriko $Var(\varepsilon)$ tipično prizamemo, da je \\
$Var(\varepsilon) = \sigma^2 I$ (homoskelastičnost + nekoreliranost). \\
Boljše modele dobimo z dodatnimi zahtevami na $\varepsilon$. \\
...
\begin{exmp} \text{} \\
  ...
  pišimo $Var(\varepsilon_i) = \sigma^2$ \\
  (vzamemo $Var(\varepsilon) = \sigma^2 I$). \\
  Tedaj je $Y_i = \mu + \varepsilon_i, \; E(Y_i) = \mu, Var(Y_i) = \sigma^2$ ($\implies$ NEP vzorčenje). \\
...
Iz \\
$\begin{bmatrix}X^1 & \dots & X^d\end{bmatrix} = \begin{bmatrix}S^1 & \dots & S^d\end{bmatrix}
\begin{bmatrix}
...
$X \hat{\beta} = S S^T y$ (pravokotna projekcija na $X, S S^T$ projektor: \\
$(S S^T) (S S^T) = S S^T$). \\
...
Zato lahko izrazimo
\begin{align*}
  E(VKR) &= E\left(tr\left((y - X \hat{\beta}) (y - X \hat{\beta})^T\right)\right) \\
  &= tr\left(Var(y - X \hat{\beta})\right) \\
  &= tr\left(Var((I - S S^T) y)\right) \\
  &= tr\left((I - S S^T) Var(Y) (I - S S^T)^T\right) \\
  &= \sigma^2 tr(I) + \sigma^2 tr\left(S S^T\right) \\
  &= \sigma^2 (n-d).
\end{align*}
Normalni standardni linearni model \\
$Y = X \beta + \varepsilon$, kjer je $\varepsilon \in N(0, \sigma^2 I)$. \\
...
\begin{itemize}
  ...
  \item dopolnimo $S$ do ortogonalne matrike $Q = \begin{bmatrix}S^{'} & S\end{bmatrix} \in O^{n \times n}$
    ...
    \begin{align*}
      ...
      &\stackrel{\text{projekcija}}{\longeq{30pt}} \langle (I - S S^T) y, y \rangle \\
      &\stackrel{\text{ortog.}}{\longeq{20pt}} \langle Q^T (I - S S^T) (Q Q^T) y, Q^T y \rangle
    \end{align*}
    \begin{enumerate}[label=(\roman*)]
...
\begin{align*}
  ...
  &\propto (\sigma^2)^{-\frac{n}{2}-1} e^{-\frac{VKR}{2 \sigma^2}} (\sigma^2)^{\frac{d}{2}}
    \cdot \sqrt{det\left(\frac{X^T X}{\sigma^2}\right)}
    \cdot e^{-\frac{1}{2} \left\langle \frac{X^T X}{\sigma^2} (\beta - \hat{\beta}), \beta - \hat(\beta)\right\rangle}.
\end{align*}
$\frac{X^T X}{\sigma^2} = (\text{var-kovar})^{-1}$. \\
Vidimo: $\int_{\sigma^2} \int_{\beta \in \R^d} \dots < \infty$, če $n-d > 0$. \\
Sledi, da imamo pri $n-d > 0$ pravo aposteriorno porazdelitev
\begin{equation*}
...
To pomeni
\begin{equation*}
  \sum_{i=1}^{\nu} N(0, \psi^{-1})^2 \stackrel{\text{neodv}}{\longeq{20pt}}
...
Pri standardni linearni regresiji sta pomembni (med drugim) 2 vprašanji,
ki jih je mogoče obravnavati s pomočjo hierarhičnih modelov:
\begin{itemize}
  \item pri $m$ homoskedastičnih skupinah $(y_j \mid \dots) \sim N(X_j \beta, \sigma^2 I)$ nas zanima
    ali so pripadajoči parametri $\beta$ med seboj različni,
  \item v \sn{eni} skupini $(y \mid \dots) \sim N(X \beta, \sigma^2 I)$ nas zanima,
    katere komponente parametra $\beta$ so različne od 0.
\end{itemize}
\underline{Problem $\beta_1 = \beta_2 = \dots = \beta_m \in \R^d$ za $m$ homoskedastičnih modelov $N(X_j \beta, \sigma^2 I)$} \\
V hierarhičnem modelu nastopajo parametri
\begin{itemize}
  \item hiperparameter $(\beta, \Sigma) = \phi \in \R^d \times P(d)$,
  \item populacijski parameter $(\beta_1 \dots \beta_m, \sigma^2) = \vartheta \in \R^d \times \dots \times \R^d \times (0, \infty)$. 
\end{itemize}
\underline{Shema vzorčenja} \\
$(\beta, \Sigma) \to (\beta_1 \dots \beta_m, \sigma^2) \to (y_1 \dots y_m)$;
\begin{itemize}
  \item $(\beta, \Sigma) \in N(\beta_0, T_0) \times InvWish(w, \Phi)$; \\
    $\beta_0 \in \R^d, T_0 \in P(d), w > d-1, \Phi \in P(d)$ \\
    hiperapriorna poradzelitev,
  \item $(\beta_1 \dots \beta_m, \sigma^2) \in N(\beta, \Sigma) \times \dots \times N(\beta, \Sigma) \times InvGama(a, b)$ \\
    populacijska apriorna porazdelitev,
  \item $(y_1 \dots y_m) \in N(X_1 \beta_1, \sigma^2 I) \times \dots \times N(X_m \beta_m, \sigma^2 I)$ \\
    vzorčna porazdelitev.
\end{itemize}
Formalno imamo gostote \\
$f(\beta, \Sigma) = f(\beta_1 \dots \beta_m, \sigma^2 \mid \beta, \Sigma)
= f(\beta_1 \dots \beta_m \mid \beta, \Sigma) \cdot f(\sigma^2)$ in \\
$f(y_1 \dots y_m \mid \beta_1 \dots \beta_m, \sigma^2, \beta, \Sigma)
= f(y_1 \dots y_m \mid \beta_1 \dots \beta_m, \sigma^2) \\
= \prod_{j=1}^{m} f_{N(X_j \beta_j, \sigma^2 I)} (y_j)$. \\
Spomnimo se abstraktnega hierarhičnega modela, kjer \\
$f(x \mid \vartheta, \phi) = f(x \mid \vartheta)$ in \\
$f(\vartheta, \phi) = f(\vartheta \mid \phi) \cdot f(\phi)$. \\
Tedaj je
\begin{equation*}
  f(\vartheta \mid \phi, x) = \frac{f(x \mid \vartheta, \phi) f(\vartheta \mid \phi)}{f(x \mid \phi)}
\end{equation*}
\begin{align*}
  f(\phi \mid \vartheta, x) &= \frac{f(x \mid \vartheta, \phi) f(\phi \mid \vartheta)}{f(x \mid \vartheta)} \\
  &\stackrel{f(x \mid \vartheta, \phi) = f(x \mid \vartheta)}{=} f(\vartheta) \\
  &\stackrel{\text{Bayes}}{\longeq{20pt}} \frac{f(\vartheta \mid \phi) f(\phi)}{f(\vartheta)}.
\end{align*}
Implementiramo
\begin{align*}
  f(\beta_1 \dots \beta_m, \sigma^2 \mid \beta, \Sigma, y_1 \dots y_m)
  &\propto \left(\prod_{j=1}^m f_{N(X_j \beta_j, \sigma^2 I)}(y_j) \cdot f_{N(\beta, \Sigma)}(\beta_j)\right)
  \cdot f_{InvGama(..)}(\sigma^2) \\
  &\propto \prod_{j=1}^{m} (\sigma^2)^{-\frac{n_j}{2} - 1} \cdot e^{-\frac{1}{2 \sigma^2} \lVert y_j - X_j \beta_j \rVert^2}.
\end{align*}
Prepoznamo \sn{polkonjugirano} apriorno porazdelitev; sledi
\begin{equation*}
  f(\beta_1 \dots \beta_m \mid \sigma^2, \beta, \Sigma, y_1 \dots y_m)
  = \prod_{j=1}^{m} f_{N\left(\beta^{(1)}, \Sigma_j^{(1)}\right)}(\beta_j);
\end{equation*}
kjer
\begin{itemize}
  \item $\Sigma_j^{(1)} = \Sigma_j^{-1} + \frac{X_j^T X_j}{\sigma^2}$ in
  \item $\beta_j^{(1)} = \Sigma_j^{(1)} \cdot \left(\Sigma^{-1} \beta + \frac{X_j^T X_j}{\sigma^2}\right)$ in
\end{itemize}
\begin{align*}
  &f(\sigma^2 \mid \beta_1 \dots \beta_m, \beta, \Sigma, y_1 \dots y_m) \dots \\
  &InvGama\left(a + \sum_{j=1}^m \frac{n_j}{2}, b + \frac{1}{2} \sum_{j=1}^{m} \lVert y_j - X_j \beta_j \rVert^2\right)
\end{align*}
DN:
\begin{itemize}
  \item $f(\beta \mid \Sigma, \beta_1 \dots \beta_m, \sigma^2)$,
  \item $f(\Sigma \mid \beta, \beta_1 \dots \beta_m, \sigma^2)$.
\end{itemize}


% 19. predavanje: 16.1.

Vprašanje neničelnosti regresijskih koeficientov v \sn{frekventističnem} modelu $y \sim N(X \beta, \sigma^2 I)$. \\
...
Tu sta $Z_j^{'}$ in $z_j^{'}$ \sn{preostanka} komponent vektorjev $Z$ in $z$. \\
